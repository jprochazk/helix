...
fn reload(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    reload_impl(cx, args, event, false)
}

fn force_reload(cx: &mut compositor::Context, args: Args, event: PromptEvent) -> anyhow::Result<()> {
    reload_impl(cx, args, event, true)
}
    ...
    for (doc_id, view_ids) in docs_view_ids {
        ...
        if !force && doc.is_modified() {
            ...
            cx.editor.set_error(format!(
                "Buffer {:?} has unsaved changes (use :reload-all! to discard them)",
                name
            ));
            continue;
        }
...
pub const TYPABLE_COMMAND_LIST: &[TypableCommand] = &[
    ...
    TypableCommand {
        name: "reload-all",
        aliases: &["rla"],
        doc: "Reload all documents from the source files. Skips buffers with unsaved changes.",
        fun: reload_all,
        completer: CommandCompleter::none(),
        signature: Signature {
            positionals: (0, Some(0)),
