...
| `:reload`, `:rl` | Reload from the source file. Fails if the buffer has unsaved changes. |
| `:reload!`, `:rl!` | Discard changes and reload from the source file. |
| `:reload-all`, `:rla` | Reload all documents from the source files. Skips buffers with unsaved changes. |
